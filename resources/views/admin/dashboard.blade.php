...
<body class="font-sans antialiased bg-gradient-to-br from-gray-50 via-white to-gray-100" x-data="dashboardApp()">
    ...
    <main class="pt-16 lg:pl-64 min-h-screen transition-all duration-300 ease-in-out">
        <div class="p-6">
            ...
            <!-- Laporan Stock -->
            <div class="bg-white/90 glass rounded-xl shadow-sm border border-gray-200/70 overflow-hidden mb-8">
                <div class="px-6 py-4 border-b border-gray-200/70 flex items-center justify-between bg-gray-50/50">
                    <h3 class="text-lg font-semibold text-gray-800">Laporan Stock</h3>
                    <a href="/admin/barang" class="text-sm text-primary-600 hover:text-primary-800 transition-slow">
                        Lihat Semua <i class="fas fa-chevron-right ml-1"></i>
                    </a>
                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-red-800">Stock Habis</p>
                                    <p class="text-2xl font-bold text-red-900">{{ count($stockReport['out_of_stock_items'] ?? []) }}</p>
                                </div>
                        ...
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-yellow-800">Stock Menipis</p>
                                    <p class="text-2xl font-bold text-yellow-900">{{ count($stockReport['low_stock_items'] ?? []) }}</p>
                                </div>
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-green-800">Total Barang</p>
                                    <p class="text-2xl font-bold text-green-900">{{ $stats['total_items'] ?? 0 }}</p>
                                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200/70">
                            <thead class="bg-gray-50/80">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Stock Tersedia</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Status</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200/70">
                                @php
                                    $criticalItems = array_merge(
                                        $stockReport['out_of_stock_items'] ?? [],
                                ...
                                <tr class="table-row-hover transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item['code'] ?? $item['kode'] ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item['name'] ?? $item['nama'] ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item['category'] ?? $item['kategori'] ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item['room'] ?? $item['ruangan'] ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span class="font-medium {{ ($item['available_stock'] ?? $item['total_stock'] ?? 0) == 0 ? 'text-red-600' : 'text-yellow-600' }}">
                                            {{ $item['available_stock'] ?? $item['total_stock'] ?? 0 }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        @if(($item['available_stock'] ?? $item['total_stock'] ?? 0) == 0)
