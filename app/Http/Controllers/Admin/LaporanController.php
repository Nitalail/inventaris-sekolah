...
class LaporanController extends Controller
{
    ...
    protected function getReportStatistics()
    {
        ...
        return [
            ...
            'monthly_reports' => Report::whereMonth('report_date', $currentMonth)
            ...
            'most_active_user' => DB::table('reports')
                                 ->select('users.name', DB::raw('COUNT(reports.id) as count'))
    ...
    protected function getTransactionData($request)
    {
        return DB::table('peminjaman')
            ...
            ->when($request->date_from && $request->date_to, function($query) use ($request) {
                return $query->whereBetween('peminjaman.tanggal_pinjam', [
                    $request->date_from, 
                    $request->date_to
                ]);
            })
            ...
            ->map(function ($item) {
                ...
                $item->tanggal_kembali = $item->tanggal_kembali 
    ...
    {
        $query = Ruangan::with(['barangs' => function($q) use ($request) {
            if ($request->category) {
    ...
    protected function getRoomInventoryData($request)
    {
        $query = Ruangan::select(
            'ruangan.*',
            DB::raw('COUNT(barang.id) as total_barang_aktual'),
            DB::raw('SUM(CASE WHEN barang.kondisi = "baik" THEN 1 ELSE 0 END) as barang_baik'),
            DB::raw('SUM(CASE WHEN barang.kondisi = "rusak" THEN 1 ELSE 0 END) as barang_rusak'),
            DB::raw('SUM(CASE WHEN barang.kondisi = "hilang" THEN 1 ELSE 0 END) as barang_hilang'),
            DB::raw('GROUP_CONCAT(DISTINCT kategori.nama) as kategori_list')
        )
        ->leftJoin('barang', 'ruangan.id', '=', 'barang.ruangan_id')
        ->leftJoin('kategori', 'barang.kategori_id', '=', 'kategori.id')
        ->groupBy('ruangan.id', 'ruangan.kode_ruangan', 'ruangan.nama_ruangan', 
                 'ruangan.kapasitas', 'ruangan.jumlah_barang', 'ruangan.status', 
                 'ruangan.deskripsi', 'ruangan.created_at', 'ruangan.updated_at');
        ...
        $rooms = $query->get()->map(function ($room) {
            $room->persentase_kapasitas = $room->kapasitas > 0 
                ? round(($room->total_barang_aktual / $room->kapasitas) * 100, 2) 
                : 0;
    ...
    {
        $query = Ruangan::select(
            DB::raw('COUNT(*) as total_ruangan'),
            DB::raw('SUM(CASE WHEN status = "aktif" THEN 1 ELSE 0 END) as ruangan_aktif'),
            DB::raw('SUM(CASE WHEN status = "perbaikan" THEN 1 ELSE 0 END) as ruangan_perbaikan'),
            DB::raw('SUM(CASE WHEN status = "tidak_aktif" THEN 1 ELSE 0 END) as ruangan_tidak_aktif'),
            DB::raw('SUM(kapasitas) as total_kapasitas'),
            DB::raw('SUM(jumlah_barang) as total_barang_terdaftar')
        );
        ...
        $stats->persentase_penggunaan = $stats->total_kapasitas > 0 
            ? round(($totalBarangAktual / $stats->total_kapasitas) * 100, 2) 
            : 0;
